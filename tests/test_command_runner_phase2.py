...
from cmdorc.command_runner import CommandRunner, CommandStatus, RunState, RunResult
...
@pytest.mark.asyncio
async def test_remaining_edge_cases():
    # 1. Template resolution KeyError (lines 62–64)
    
    cfg = CommandConfig(name="ph", command="ls", triggers=["go"])
    runner = CommandRunner([cfg])
    runner.vars = {"good": "ok"}
    with pytest.raises(KeyError):
        runner._resolve_template("hello {{bad}}")

    # 2. on_trigger / off_trigger (69–81)
    called = asyncio.Event()
    def cb(_): called.set()
    runner.on_trigger("test", cb)
    await runner.trigger("test")
    ...
    # 3. Trigger with no handlers → debug log (119–120)
    await runner.trigger("nonexistent-trigger")

    # 4. ValueError in _task_completed when state still RUNNING (185)
    # This should never happen — but we can trigger the log path
    # (or just accept it's unreachable — but let's be pedantic)
    result = RunResult()
    result.command_name = "Test"
    result.state = RunState.RUNNING  # still RUNNING!
    with pytest.raises(ValueError):
        runner._task_completed("Test", result)

    # 5. Error output logging on cancel/fail (330, 340–341, 359, 388)
    cfg = CommandConfig(name="Err", command="echo error >&2; exit 1", triggers=["go"])
    runner = CommandRunner([cfg])
    proc = AsyncMock()
    proc.communicate.return_value = (b"", b"BOOM\n")
    proc.returncode = 1
    ...
    # 6. get_result with bad run_id → ValueError (400)
    with pytest.raises(ValueError):
        runner.get_result("Err", "00000000-0000-0000-0000-000000000000")

    # 7. validate_templates strict=True (434, 447)
    runner2 = CommandRunner([CommandConfig(name="Bad", command="{{x}}", triggers=["go"])])
    with pytest.raises(ValueError):
        runner2.validate_templates(strict=True)

@pytest.mark.asyncio
async def test_coverage_edge_cases(mock_success_proc, caplog):
    # Setup runner with bad template for validation tests
    bad_cfg = CommandConfig(name="BadTemplate", command="{{missing}}", triggers=["go"])
    runner = CommandRunner([bad_cfg, CommandConfig(name="History", command="echo ok", triggers=["hist"], keep_history=3)])

    # 62-64: KeyError in _resolve_template
    with pytest.raises(KeyError, match="Unresolvable variable"):
        runner._resolve_template("hello {{badvar}}")

    # 69-81: on_trigger and off_trigger
    called = False
    def cb(_): nonlocal called; called = True
    runner.on_trigger("callback_test", cb)
    await runner.trigger("callback_test")
    assert called
    runner.off_trigger("callback_test", cb)
    called = False
    await runner.trigger("callback_test")
    assert not called

    # 119-120: Trigger with no handlers (debug log)
    await runner.trigger("no_handlers_here")

    # 139: Cycle detection warning
    cycle_cfg = CommandConfig(name="Cycle", command="echo loop", triggers=["loop", "command_success:Cycle"])
    runner = CommandRunner([cycle_cfg])
    with caplog.at_level(logging.WARNING):
        await runner.trigger("loop")
        await asyncio.sleep(0.1)
        assert "Trigger cycle detected" in caplog.text

    # 185: ValueError if state still RUNNING in _task_completed
    dummy_result = RunResult()
    dummy_result.command_name = "Dummy"
    dummy_result.state = RunState.RUNNING  # Force invalid state
    with pytest.raises(ValueError, match="completed but result state is still RUNNING"):
        runner._task_completed("Dummy", dummy_result)

    # 314-315, 340-341: proc.kill() + await wait in CancelledError, and logger.error on cancel with output
    proc = create_long_running_proc()
    proc.communicate.side_effect = lambda: (b"", b"ERROR OUTPUT\n")  # Add output for logger.error
    with patch("asyncio.create_subprocess_shell", return_value=proc):
        cfg = CommandConfig(name="CancelWithErr", command="sleep 100", triggers=["start"], cancel_on_triggers=["stop"])
        runner = CommandRunner([cfg])
        await runner.trigger("start")
        assert await runner.wait_for_running("CancelWithErr", timeout=2.0)
        await runner.trigger("stop")
        assert await runner.wait_for_cancelled("CancelWithErr", timeout=3.0)
        await asyncio.sleep(0.5)
        assert runner.get_status("CancelWithErr") == CommandStatus.CANCELLED
        # logger.error hit due to output

    # 359: Trimming history debug
    with patch("asyncio.create_subprocess_shell", return_value=mock_success_proc):
        for _ in range(5):  # More than keep_history=3
            await runner.trigger("hist")
            await asyncio.sleep(0.1)
        assert len(runner.get_history("History")) == 3  # Trims to 3

    # 388: get_status with run_id
    run_id = runner.get_result("History").run_id
    assert runner.get_status("History", run_id=run_id) == CommandStatus.SUCCESS

    # 400: ValueError in get_result with bad run_id
    with pytest.raises(ValueError, match="Run not found"):
        runner.get_result("History", "fake-run-id")

    # 434, 447: validate_templates strict=True raise
    with pytest.raises(ValueError, match="Unresolved templates"):
        runner.validate_templates(strict=True)

    # 470, 476: Helper methods (wait_for_running, wait_for_idle, wait_for_cancelled)
    await runner.trigger("start")  # Assume a command
    assert await runner.wait_for_running("Cancelable", timeout=1.0)
    assert await runner.wait_for_idle("NonExistent", timeout=1.0)  # IDLE by default
    await runner.trigger("stop")
    await asyncio.sleep(0.1)
    assert await runner.wait_for_cancelled("Cancelable", timeout=1.0)